...
class BlastFurnaceRecipe(
    ...
    override fun addWidgets(widgets: WidgetHolder) {
        ...
        if(recipe.value is BlastFurnaceRecipe) {
            val blastRecipe = recipe.value as BlastFurnaceRecipe

            widgets.addText(
                Text.literal((blastRecipe.heat().toString())).append(" ").append(Text.translatable("techreborn.jei.recipe.heat")),
                xSize - 4,
                1,
                ColorHelper.Argb.getArgb(255, 64, 64, 64),
                false
        ...
        widgets.add(ProgressBarWidget(47, 30, recipe.value.time() * 50.0, GuiBuilder.ProgressDirection.RIGHT))
